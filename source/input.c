  ...
  /* Additional SCF parameters: */
    ...
    if (flag1 == _TRUE_){
      if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
        ...
        if (pba->scf_parameters[0] >= 0.)
    	pba->y_phi_ini_scf = 5.*pba->theta_phi_ini_scf*pow(1.-2.*y2_phi_scf(pba,Omega_phi,theta_phi,y1_phi)*pow(Omega_phi,0.5)*cos_scf(pba,0.5*theta_phi)/pow(5.*pba->theta_phi_ini_scf,2.),0.5);
        else
