  ...
  */
  ...
  /* Additional SCF parameters: */
    ...
    if (pba->scf_parameters[0] < 0.){
        ...
        masstohubble_ini = 1.564e29*pow(ppr->a_ini_over_a_today_default,2.)*pow(10.,pba->scf_parameters[1])/
        (pow(pba->Omega0_g+pba->Omega0_ur,0.5)*pba->H0);
        ...
        if (pba->scf_parameters[0] > 1.e2){
            ...
            Omega_ini = pba->scf_parameters[pba->scf_tuning_index]+
            log(pba->Omega0_scf*ppr->a_ini_over_a_today_default/(pow(aosc,3.)*(pba->Omega0_g+pba->Omega0_ur)));
...
//        if (pba->scf_parameters[0] > 0.)
//        printf(" -> ratio = %1.6e, lambda_scf = %1.2e, tuning = %1.6e\n",
